...
// Supabase client
const supabaseClient = (0, supabase_js_1.createClient)(SUPABASE_URL, SUPABASE_SERVICE_KEY, {
    auth: {
            ...
            try {
                ...
                if (value.messages && value.messages.length > 0) {
                    ...
                    for (const message of value.messages) {
                        await processIncomingMessage(supabase, message, value.metadata?.phone_number_id, value.contacts?.[0]?.profile?.name);
                    }
                }
...
server.post("/send-whatsapp-message", async (request, reply) => {
    try {
        ...
        const { user_id, customer_phone, message, type = "text", category = "utility", is_promotional = false, template_name, template_params = [], header_params = [], template_buttons = [], media_id, media_ids, caption, filename, } = body;
        ...
        if (!user_id ||
            ...
            if (type === "text")
                ...
                missingField = "template_name";
            else
                missingField = "media_id or media_ids";
        ...
        if (type === "template") {
            ...
            for (const param of header_params) {
                ...
                if (param.type === "currency" &&
                    ...
                    return reply.code(400).send({
                        error: "currency header parameter missing required fields (fallback_value, code, amount_1000)",
                    });
                }
            ...
            for (const param of template_params) {
                ...
                if (param.type === "currency" &&
                    (!param.currency ||
                        ...
                        !param.currency.fallback_value)) {
                    return reply.code(400).send({
                        error: "currency parameter missing required fields (fallback_value, code, amount_1000)",
                    });
                }
                if (param.type === "date_time" &&
                    (!param.date_time || !param.date_time.fallback_value)) {
        ...
        if (mediaIdsToProcess.length > 0) {
            if (useTemplate) {
                return reply.code(400).send({
                    error: "Media messages cannot be sent using templates. Ensure you're within the 24-hour messaging window.",
                });
            }
            ...
            console.log(`[DEBUG] Starting to process ${mediaIdsToProcess.length} media items`);
            processedMedia = await Promise.all(mediaIdsToProcess.map(async (mediaId, index) => {
                ...
                console.log(`[DEBUG] Media ${index + 1} URL fetched: ${media_download_url}, format: ${mediaFormat}`);
                console.log(`[DEBUG] Media ${index + 1} metadata:`, JSON.stringify(mediaUrlData, null, 2));
                // Download media
                console.log(`[DEBUG] Downloading media ${index + 1} for storage:`, media_download_url);
                const mediaResponse = await fetch(media_download_url, {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                    },
                ...
                console.log(`[DEBUG] Processed media ${index + 1} ID for sending: ${effectiveMediaId}`);
                return { effectiveMediaId, storedMediaUrl, mediaFormat };
            }));
            console.log(`[DEBUG] All media processed: ${JSON.stringify(processedMedia.map((p) => ({
            ...
            console.log(`[DEBUG] Unique media formats found: ${JSON.stringify(uniqueFormats)}`);
            if (uniqueFormats.length > 1) {
                throw new Error("Mixed media formats not supported in a single request");
            }
            const actualType = uniqueFormats[0];
            console.log(`[DEBUG] Actual media type: ${actualType}, expected: ${type}`);
            if (actualType !== type) {
                throw new Error(`Media format mismatch: expected ${type}, got ${actualType}`);
            }
        }
        let whatsappPayload;
        let allResults = [];
        let allMessageIds = [];
        let templateStoredMediaUrl = null;
        let templateMediaType = null;
        let headerMediaObject = null;
        if (!useTemplate) {
            console.log(`[DEBUG] Building WhatsApp payload for type: ${type}, media count: ${processedMedia.length}`);
            if (type === "text") {
                // Free-form text
                ...
                console.log(`[DEBUG] Media payload for ${type}, caption: "${effective_caption}", media count: ${processedMedia.length}`);
                ...
                if (processedMedia.length === 1) {
                    ...
                    console.log(`[DEBUG] Single media payload ID: ${singleMedia.effectiveMediaId}`);
                }
                else if (processedMedia.length > 1 && type === "image") {
                    console.log(`[DEBUG] Multiple images detected (${processedMedia.length}), preparing separate payloads`);
                    ...
                    console.log(`[DEBUG] Multiple image payloads prepared: ${JSON.stringify(multiplePayloads.map((p) => p[type].id), null, 2)}`);
                ...
                console.log(`[DEBUG] Audio payload ID: ${singleMedia.effectiveMediaId}`);
            }
                ...
                console.log(`[DEBUG] Document payload ID: ${singleMedia.effectiveMediaId}`);
            }
            else {
                return reply.code(400).send({
            ...
            console.log(`[DEBUG] Final WhatsApp payload: ${JSON.stringify(whatsappPayload, null, 2)}`);
        }
        else {
            // Template
            console.log("Fetching template with name:", template_name, "for agent_id:", agent.id);
            ...
            const storedHeader = storedComponents.find((c) => c.type.toLowerCase() === "header");
            const storedBody = storedComponents.find((c) => c.type.toLowerCase() === "body");
            const storedButtons = storedComponents.filter((c) => c.type.toLowerCase() === "button");
            // Require media_header if template has media header
            if (storedHeader &&
                ["IMAGE", "VIDEO", "DOCUMENT"].includes(storedHeader.format)) {
                console.log("Template header details:", JSON.stringify(storedHeader, null, 2));
                if (!media_header) {
                    if (storedHeader.example &&
                        ...
                        console.log("Using example media link for header:", media_header.link);
                    }
                    else {
                        console.log("Media header required but not provided for format:", storedHeader.format);
                        return reply.code(400).send({
                            error: "Media header required for this template (format: " +
                                storedHeader.format +
                                ")",
                        });
                    }
            ...
            if (media_header &&
                (!storedHeader ||
                    !["IMAGE", "VIDEO", "DOCUMENT"].includes(storedHeader.format))) {
                console.log("Media header provided but template header format is:", storedHeader ? storedHeader.format : "none");
                return reply.code(400).send({
            ...
            if (storedHeader && storedHeader.format?.toUpperCase() === "TEXT") {
                const requiredHeaderParams = storedHeader.example?.header_text_named_params?.length || 0;
            ...
            const requiredBodyParams = storedBody?.example?.body_text_named_params?.length || 0;
            ...
            if (media_header) {
                ...
                let effectiveHeaderId = null;
                let downloadUrl = null;
                let mimeType = null;
                if (id) {
                    // For existing ID, fetch details to validate and get download URL for storage
                    const mediaUrlResponse = await fetch(`https://graph.facebook.com/v23.0/${id}`, {
                        method: "GET",
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
                ...
                console.log("Downloading media for template header storage:", downloadUrl);
                ...
                if (link) {
                    ...
                    if (!storageResponse.ok) {
                        ...
                        const storageMimeType = mimeType ||
                        ...
                        if (agent && agent.agent_prefix) {
                            try {
                                ...
                                const fileName = `template_header_${timestamp}_${crypto_1.default.randomUUID()}.${fileExt}`;
                                const filePath = `${agent.agent_prefix}/outgoing/${fileName}`;
                                const { data: uploadData, error: uploadError } = await supabaseClient.storage
                                    .from("whatsapp-media")
                                ...
                                if (!uploadError && uploadData) {
                                    ...
                                    console.log(`Template header media stored: ${templateStoredMediaUrl}`);
                                }
                                else {
                                ...
                                console.error("Storage error for template media:", storageError);
                            }
                        }
                    }
                }
            ...
            const templateLanguageCode = typeof templateData.body.language === "string"
                    ...
                    parameters: [
                        {
                            ...
                            [templateMediaType]: headerMediaObject,
                        },
                    ],
                });
            }
            ...
            if (storedHeader &&
                ...
                const headerParameters = header_params
                    .map((param, index) => {
                    const paramName = storedHeader.example?.header_text_named_params?.[index]
                        ?.param_name;
                    switch (param.type) {
                        case "text":
                            return {
            ...
            if (template_params.length > 0) {
                ...
                for (let i = 0; i < template_params.length; i++) {
                    ...
                    const paramName = storedBody?.example?.body_text_named_params?.[i]?.param_name;
                    let paramObj;
                    switch (param.type) {
                        case "text":
                            paramObj = {
            ...
            console.log("Constructed template payload:", JSON.stringify(whatsappPayload, null, 2));
            // Debug logging for template payload
            console.log("Template data body:", JSON.stringify(templateData.body, null, 2));
            console.log("Constructed WhatsApp payload:", JSON.stringify(whatsappPayload, null, 2));
        }
        // Send to WhatsApp
        if (Array.isArray(whatsappPayload)) {
            console.log(`[DEBUG] Sending ${whatsappPayload.length} separate messages`);
            for (let i = 0; i < whatsappPayload.length; i++) {
                const singlePayload = whatsappPayload[i];
                console.log(`[DEBUG] Sending message ${i + 1}/${whatsappPayload.length}: ${JSON.stringify(singlePayload, null, 2)}`);
                ...
                console.log(`[DEBUG] Response for message ${i + 1}:`, JSON.stringify(result, null, 2));
            ...
            console.log(`[DEBUG] Sending single payload: ${JSON.stringify(whatsappPayload, null, 2)}`);
            const response = await fetch(`https://graph.facebook.com/v23.0/${phoneNumberId}/messages`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
            ...
            if (!response.ok) {
                console.error("WhatsApp API error details:", JSON.stringify(result, null, 2));
            }
            if (!response.ok) {
            ...
            if (useTemplate) {
                const { data: newCredits, error: deductError } = await supabaseClient.rpc("deduct_credits", {
                    p_agent_id: agent.id,
                    p_amount: 0.01,
                });
        ...
        if (Array.isArray(whatsappPayload) && type === "image") {
            console.log(`[DEBUG] Inserting ${processedMedia.length} separate image records`);
            for (let i = 0; i < processedMedia.length; i++) {
                const media = processedMedia[i];
                const effective_caption = (caption || message || "").trim();
                const singleTimestamp = new Date(Date.parse(messageTimestamp) + i * 100).toISOString();
                ...
                if (msgError) {
                    console.error(`[DEBUG] Error inserting image message ${i + 1}:`, msgError);
                }
                else {
                    storedMessagesCount++;
                }
            ...
            const bodyComponent = templateData.body.components?.find((c) => c.type.toLowerCase() === "body");
            if (bodyComponent && bodyComponent.text) {
                ...
                for (let i = 0; i < dynamicData.body_params.length; i++) {
                    ...
                    renderedBody = renderedBody.replace(new RegExp(escapeRegExp(placeholder), "g"), paramValue);
                }
            }
            ...
            let stored_message = typeof message === "string" ? message : JSON.stringify(message);
            const { error: msgError } = await supabaseClient
                .from(messagesTable)
                .insert({
                customer_id: customer.id,
        ...
        console.log(`[DEBUG] Messages sent successfully: ${allMessageIds.length} WhatsApp IDs, ${storedMessagesCount} DB records`);
        return reply.code(200).send({
            success: true,
...
server.post("/upload-media", async (request, reply) => {
    ...
    try {
        ...
        const invalidFiles = files.filter((file) => {
            const maxForType = allowedMediaTypes[file.type];
            if (maxForType === undefined) {
                console.warn(`Unsupported MIME type: ${file.type}`);
                return true;
            }
            if (file.size > maxForType) {
                console.warn(`File too large: ${file.name} (${file.size} bytes) exceeds ${maxForType} bytes for ${file.type}`);
                return true;
            }
            return false;
        });
        if (invalidFiles.length > 0) {
            console.error("❌ Invalid files:", invalidFiles.map((f) => ({ name: f.name, size: f.size, type: f.type })));
            return reply.code(400).send({
                error: "Some files are too large or unsupported type. Limits: Images/Stickers (5MB/500KB), Documents (100MB), Audio/Video (16MB).",
            });
        }
        ...
        const uploadPromises = files.map(async (file, index) => {
            ...
            console.log(`⏱️ Cloud API upload for file ${index + 1} took ${uploadDuration}ms`);
            if (!uploadResponse.ok) {
                const errorData = await uploadResponse.text();
                console.error(`❌ Media upload failed for file ${index + 1}:`, errorData);
                return {
            ...
            // Fetch media URL
            let media_download_url = null;
            const mediaUrlResponse = await fetch(`https://graph.facebook.com/${version}/${media_id}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            ...
            if (mediaUrlResponse.ok) {
                ...
                console.log(`Media URL for file ${index + 1} fetched:`, media_download_url);
            }
            else {
                console.error(`Failed to fetch media URL for file ${index + 1}:`, await mediaUrlResponse.text());
            }
            // Download and upload to Supabase
            let supabasePublicUrl = null;
            if (media_download_url) {
                try {
                    console.log(`🔄 Attempting to download preview blob for file ${index + 1} from:`, media_download_url.substring(0, 50) + "...");
                    const blobResponse = await fetch(media_download_url, {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
                    });
                    console.log(`📥 Blob response for file ${index + 1}: status ${blobResponse.status}, ok: ${blobResponse.ok}`);
                    if (blobResponse.ok) {
                        const blob = await blobResponse.blob();
                        console.log(`📦 Blob downloaded for file ${index + 1}: size ${blob.size} bytes, type ${blob.type}`);
                        const timestamp = Date.now();
                        ...
                        if (!uploadError && uploadData) {
                            ...
                            console.log(`✅ Preview uploaded to Supabase for file ${index + 1}: ${supabasePublicUrl}`);
                        }
                        else {
                            console.error(`❌ Failed to upload preview to Supabase for file ${index + 1}:`, uploadError);
                        }
                    }
                    else {
                        const errorText = await blobResponse.text();
                        console.error(`❌ Failed to download blob for preview for file ${index + 1}: status ${blobResponse.status}, response:`, errorText);
                    }
                }
                catch (blobError) {
                    console.error(`❌ Error downloading blob for preview for file ${index + 1}:`, blobError);
                }
            }
            ...
            else if (["pdf", "txt", "doc", "docx", "xls", "xlsx", "ppt", "pptx"].includes(extension)) {
                mediaType = "document";
            }
            console.log(`🏷️ Determined media type for file ${index + 1}: ${mediaType} (extension: ${extension})`);
            return {
                media_id: media_id,
        ...
        if (failedUploads.length > 0) {
            console.error("❌ Some uploads failed:", failedUploads.map((f) => f.error));
        }
        ...
        console.log(`🎉 ${successfulUploads.length}/${files.length} files uploaded successfully`, successfulUploads.map((u) => ({ id: u.media_id, type: u.media_type })));
