...
- **Manual Step**:
  ...
  2. Inspect database:
     ...
     SELECT ai_balance, credits FROM agents WHERE id = <agent_id>;
...
- **Expected Outcome**:
  ...
  - 2.0x actual DeepSeek token cost deducted from `agents.ai_balance` (WhatsApp template `credits` remains untouched).
  - Agent UI shows updated balance without displaying any 2x multiplier badge or text.
  - Socket.IO broadcast `ai_balance_updated` emitted and balance updated in agent UI.
...
- **Expected Outcome**: Inbound message persisted; no outbound AI reply created; `ai_balance` unchanged.

#### TC-AI-03: Insufficient Agent AI Balance Handling (`ai_balance <= 0`)
- **Objective**: Verify that agents with depleted AI balance (<= 0) do not trigger DeepSeek calls or crash the webhook.
- **Preconditions**: Set `agents.ai_balance = 0.00`.
- **Manual Step**:
  1. Send inbound WhatsApp message with `ai_enabled = true`.
  2. Check backend server console logs.
- **Expected Outcome**: Warning logged: `Agent has depleted balance ($0.0000) for AI reply`; webhook responds HTTP 200; no `ai_balance` negative drift.

#### TC-AI-04: Product Inquiry CRM Action (`POST /trigger-ai-response`)
- **Objective**: Verify that selecting a product in the conversation drawer dispatches an AI-generated product overview to the customer and deducts double (2.0x) the actual DeepSeek API cost from `agents.ai_balance`.
- **Manual Step**:
  ```bash
  ...
  ```
- **Expected Outcome**: HTTP 200, response contains generated product text without emojis; outbound message created; 2.0x actual DeepSeek token cost deducted from `agents.ai_balance`; template `credits` untouched; new balance returned in JSON response.
...
- **Manual Step**:
  ...
  ```
- **Expected Outcome**: HTTP 200, response contains generated service packages and pricing details without emojis; outbound message created; 2.0x actual DeepSeek token cost deducted from `agents.ai_balance`.
...
- **Expected Outcome**: Response is delivered in natural Sinhala/Singlish, contains no emojis, quotes real packages, and deducts 2.0x DeepSeek token cost from `ai_balance`.

#### TC-AI-08: Super Admin Top-Up for AI and Template Credits
- **Objective**: Verify that Super Admins can top up either DeepSeek AI Balance or WhatsApp Template Credits independently, while non-admin agents cannot.
- **Manual Step**:
  1. As Admin, navigate to Admin Dashboard -> Agents.
  2. Click **Top Up** on an agent row.
  3. Select **DeepSeek AI Balance**, enter `10.00`, click **Add $10.00 to AI Balance**. Verify agent `ai_balance` increases by 10.00.
  4. Click **Top Up** again, select **WhatsApp Template Message Credits**, enter `5.00`, click **Add 5.00 Template Credits**. Verify `credits` increases by 5.00.
  5. Attempt same API call as an agent JWT:
     ```bash
     curl -X POST http://localhost:8080/add-credits \
       -H "Authorization: Bearer <AGENT_JWT_TOKEN>" \
       -H "Content-Type: application/json" \
       -d '{"agent_id": 1, "amount": 10.00, "balance_type": "ai"}'
     ```
- **Expected Outcome**: Admin top-ups succeed and update respective balances immediately with websocket emission; agent direct request is rejected with HTTP 403 `Access denied. Admin role required.`
...
| `TC-AI-01` | Inbound DeepSeek Autonomous Reply | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Contextual reply & 2x API cost deducted from ai_balance |
| `TC-AI-02` | AI Disabled Per-Customer Toggle | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | No AI reply generated |
| `TC-AI-03` | Insufficient AI Balance Guard | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Skipped without failure |
...
| `TC-AI-07` | Multilingual Inbound Response | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Natural Sinhala/Singlish delivery |
| `TC-AI-08` | Super Admin Balance Top-Up | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Admin top-up succeeds; agent request rejected 403 |
...
| `TC-FE-TOPUP-01`| Top Ups Tab Navigation & KPI Summary | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Super Admin can navigate to Top Ups tab and see total AI balance & credits summary |
| `TC-FE-TOPUP-02`| Agent Search & Quick Top Up | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Search filters agents by name/email/prefix and opens Top Up modal with correct balances |
| `TC-FE-TOPUP-03`| Agents Tab Separation | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Agents table and mobile cards do not display balance columns or Top Up action buttons |
| `TC-FE-HEADER-01`| Header-Free Super Admin Pages | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | Desktop layout contains no redundant top header; mobile uses floating hamburger toggle |
| `TC-FE-ADM-UX-01`| Super Admin Style Guide Alignment | [Tester Name] | YYYY-MM-DD | [PASS / FAIL] | 90vw desktop bound, green/emerald brand tokens, zero purple/emojis, Syne typography |
