-- Migration: Ensure customer phone numbers are unique per agent, merge duplicates, and update table creation logic.
-- Run this to update existing tables and the creation logic for new agents.

BEGIN;

-- 1. Helper function to merge duplicates and clean phone numbers
CREATE OR REPLACE FUNCTION merge_duplicate_customers(p_agent_prefix TEXT)
RETURNS VOID AS $$
DECLARE
    ...
    dup_id INT;
BEGIN
    -- Normalize all phone numbers to raw digits (remove non-digits like +, spaces, dashes, etc.)
    EXECUTE format('UPDATE %I SET phone = regexp_replace(phone, ''\D'', '''', ''g'')', customers_table);
    
    -- Find duplicate phone numbers and aggregate their IDs.
    -- Order by last_user_message_time DESC to keep the most active customer row.
    FOR dup_rec IN
        EXECUTE format('
            SELECT phone, array_agg(id ORDER BY last_user_message_time DESC NULLS LAST, created_at DESC) as ids
            FROM %I
            GROUP BY phone
            HAVING count(*) > 1
        ', customers_table)
    LOOP
        ...
        -- Merge referencing rows for all duplicates into the keep_id
        FOREACH dup_id IN ARRAY dup_rec.ids[2:] LOOP
            -- Update messages
            IF EXISTS (SELECT FROM pg_tables WHERE tablename = messages_table AND schemaname = 'public') THEN
                EXECUTE format('UPDATE %I SET customer_id = $1 WHERE customer_id = $2', messages_table) USING keep_id, dup_id;
            END IF;
            
            -- Update orders
            IF EXISTS (SELECT FROM pg_tables WHERE tablename = orders_table AND schemaname = 'public') THEN
                EXECUTE format('UPDATE %I SET customer_id = $1 WHERE customer_id = $2', orders_table) USING keep_id, dup_id;
            END IF;
            
            -- Update appointments
            IF EXISTS (SELECT FROM pg_tables WHERE tablename = appointments_table AND schemaname = 'public') THEN
                EXECUTE format('UPDATE %I SET customer_id = $1 WHERE customer_id = $2', appointments_table) USING keep_id, dup_id;
            END IF;
            
            -- Safely delete the duplicate customer
            EXECUTE format('DELETE FROM %I WHERE id = $1', customers_table) USING dup_id;
        END LOOP;
    END LOOP;
    
    -- Add UNIQUE constraint to the phone column in the customers table
    EXECUTE format('ALTER TABLE %I DROP CONSTRAINT IF EXISTS unique_%s_phone', customers_table, customers_table);
    EXECUTE format('ALTER TABLE %I ADD CONSTRAINT unique_%s_phone UNIQUE (phone)', customers_table, customers_table);
    
    RAISE NOTICE 'Merged duplicates and enforced UNIQUE phone for agent: %', p_agent_prefix;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 2. Execute on all existing customer tables
DO $$
DECLARE
    agent_rec RECORD;
    c_table TEXT;
BEGIN
    FOR agent_rec IN SELECT agent_prefix FROM agents LOOP
        c_table := agent_rec.agent_prefix || '_customers';
        
        -- Check if customer table actually exists before attempting merge/alter
        IF EXISTS (SELECT FROM pg_tables WHERE tablename = c_table AND schemaname = 'public') THEN
            PERFORM merge_duplicate_customers(agent_rec.agent_prefix);
        END IF;
    END LOOP;
END $$;

-- Drop the helper function after migration run
DROP FUNCTION IF EXISTS merge_duplicate_customers(TEXT);

-- 3. Update the create_agent_tables function to include UNIQUE constraint on phone
...
-- Safely grant permissions only if the roles exist in the database
DO $$
BEGIN
    EXECUTE 'GRANT EXECUTE ON FUNCTION create_agent_tables(TEXT, BIGINT) TO service_role, authenticated';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Could not grant permissions on create_agent_tables: %', SQLERRM;
END $$;
