...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userRole === "admin" || userRole === "agent") {
        // Valid role login - store token and redirect to appropriate dashboard
  ...
  // Check for URL error parameters and existing token
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-cyan-50">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-1/4 left-1/4 w-2 h-2 bg-emerald-300 rounded-full opacity-20 animate-bounce"
          style={{ animationDelay: "0s" }}
        ></div>
        <div
          className="absolute top-3/4 right-1/4 w-3 h-3 bg-cyan-300 rounded-full opacity-20 animate-bounce"
          style={{ animationDelay: "1s" }}
        ></div>
        <div
          className="absolute bottom-1/4 left-3/4 w-1.5 h-1.5 bg-emerald-200 rounded-full opacity-20 animate-bounce"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute top-1/2 left-1/2 w-4 h-4 bg-cyan-200 rounded-full opacity-10 animate-ping"
        ...
        <div className="backdrop-blur-xl bg-white/95 border border-white/30 shadow-2xl rounded-3xl p-6 sm:p-8 relative overflow-hidden transform transition-all duration-500 hover:scale-[1.02] animate-pulse-slow">
          ...
          >
            ...
            <div className="relative group">
              ...
              <div
                className={`absolute right-3 sm:right-4 top-4 transition-colors duration-200 ${
                  error
                    ? "text-red-500"
                    : email
                    ? "text-emerald-500"
            ...
            <div className="relative group">
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 rounded-2xl blur opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${
                    loading || !email || !password ? "opacity-0" : ""
                  }`}
                ></div>
                ...
                <span className="relative z-10 flex items-center">
                  {loading ? (
                    ...
                    <>
                      <svg
                        className={`mr-2 h-4 w-4 sm:h-5 sm:w-5 transition-transform duration-300 ${
                          email && password ? "group-hover:translate-x-1" : ""
                        }`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
            ...
            <div className="text-center pt-4 sm:pt-6 border-t border-gray-200/20">
              <div className="flex items-center justify-center space-x-2 mb-2">
                ...
                <div
                  className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-cyan-500 rounded-full animate-pulse"
                  style={{ animationDelay: "0.5s" }}
                ></div>
              </div>
        ...
        <div className="mt-4 sm:mt-6 text-center">
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-2 sm:p-3 border border-white/30 shadow-sm">
            <p className="text-xs text-gray-600 font-medium">
              💚 WhatsApp CRM Admin Portal v1.0
            </p>
          </div>
        </div>
      </div>
