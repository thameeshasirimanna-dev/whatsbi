...
export const useAppointments = () => {
  ...
  const functionUrl = `${supabaseUrl}/functions/v1/manage-appointments`;

  const fetchAppointments = useCallback(async (params: FetchAppointmentsParams = {}) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (params.customerId !== undefined && params.customerId !== null) url.searchParams.set('customer_id', params.customerId.toString());
  ...
  const updateAppointment = useCallback(async (id: number, data: UpdateAppointmentData) => {
    try {
      setError(null);
