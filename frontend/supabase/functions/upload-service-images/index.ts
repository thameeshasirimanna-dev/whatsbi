...
serve(async (req) => {
  ...
  try {
    ...
    const { agentId, serviceId, images } = await req.json()
    ...
    // serviceId optional for create (will use root path)
    ...
    for (const image of images) {
      try {
        ...
        const filePath = serviceId ? `${serviceId}/${uniqueId}${extension}` : `${uniqueId}${extension}`
        ...
        if (uploadError) {
          ...
          for (const uploadedUrl of uploadedUrls) {
            ...
            if (parts.length > 0 && parts[0] === 'service-images') {
              const pathToDelete = parts.slice(1).join('/');
              await supabaseClient.storage.from('service-images').remove([pathToDelete]);
            }
          }
          return new Response(
